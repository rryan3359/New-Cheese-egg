...
import { buildMarketEvents, buildSessionLevels, currentSession } from "./sessions";
...
/** OKX is the sole market provider. Missing fields remain null and missing. */
...
function buildTimeframe(timeframe: Timeframe, candles: TimeframeSnapshot["candles"], source: "OKX" | "Calculated", state: DataState, latency: number | null, now: string): TimeframeSnapshot {
  ...
  const highThreshold = timeframe === "1m" || timeframe === "5m" ? 0.008 : timeframe === "15m" ? 0.012 : 0.025;
  const lowThreshold = timeframe === "1m" || timeframe === "5m" ? 0.0015 : timeframe === "15m" ? 0.003 : 0.008;
  const volatility = ratio === null ? null : ratio > highThreshold ? "High" : ratio < lowThreshold ? "Low" : "Normal";
...
export function mergeProviderPayloads(input: {
  ...
  const assets = Array.from(okxMap.keys()).map((symbol): AssetSnapshot => {
    ...
    const candlesByTimeframe = Object.fromEntries(TIMEFRAMES.map((timeframe) => [timeframe, timeframes[timeframe].candles])) as Record<Timeframe, TimeframeSnapshot["candles"]>;
    const hasCandles = TIMEFRAMES.some((timeframe) => timeframes[timeframe].candles.length > 0);
    const sessionLevels = hasCandles ? buildSessionLevels(candlesByTimeframe, nowDate) : [];
    const events = hasCandles ? buildMarketEvents(symbol, candlesByTimeframe, sessionLevels, funding.value, oiChange.value) : [];
    const strategies = hasCandles ? evaluateStrategies({
      symbol, candlesByTimeframe, sessionLevels, funding: funding.value, oiChange1h: oiChange.value,
      topRatios, globalRatios, costs: input.costs ?? DEFAULT_TRADE_COSTS, now,
    }) : [];
    return {
  ...
  const up = knownTrends.filter((asset) => asset.timeframes["4h"].trend.value === "Trend Up").length;
  const down = knownTrends.filter((asset) => asset.timeframes["4h"].trend.value === "Trend Down").length;
  const highVol = knownVolatility.filter((asset) => asset.timeframes["1h"].volatility.value === "High").length;
  const lowVol = knownVolatility.filter((asset) => asset.timeframes["1h"].volatility.value === "Low").length;
  const recentEvents = assets.flatMap((asset) => asset.events).sort((a, b) => b.occurredAt.localeCompare(a.occurredAt)).slice(0, 12);
  const scannerCount = assets.flatMap((asset) => asset.strategies).filter((strategy) => strategy.eligibleForScanner).length;
  const regime: MarketHubPayload["regime"] = recentEvents.some((event) => event.kind === "liquidity_sweep")
    ? "Liquidity Sweep"
    : knownVolatility.length && highVol >= Math.ceil(knownVolatility.length / 2)
      ? "High Volatility"
      : knownVolatility.length && lowVol >= Math.ceil(knownVolatility.length / 2)
        ? "Compression"
        : knownTrends.length && up >= Math.ceil(knownTrends.length * 0.6)
          ? "Trend Up"
          : knownTrends.length && down >= Math.ceil(knownTrends.length * 0.6)
            ? "Trend Down"
            : knownTrends.length && scannerCount === 0 ? "No Trade" : knownTrends.length ? "No Trade" : "N/A";
  const riskAlerts = assets.flatMap((asset) => {
    const alerts: string[] = [];
    if (asset.funding.value !== null && Math.abs(asset.funding.value) > 0.0005) alerts.push(`${asset.symbol.replace("USDT", "")} Funding 過熱（背景）`);
    if (asset.oiChange1h.value !== null && Math.abs(asset.oiChange1h.value) > 4) alerts.push(`${asset.symbol.replace("USDT", "")} OI 1h 異常變化（背景）`);
  ...
  return {
    success: true, updatedAt: now, cacheAgeMs: 0,
    staleExpiresAt: new Date(nowDate.getTime() + (input.staleTtlMs ?? 10 * 60_000)).toISOString(),
    assets, fearGreed: input.fearGreed, breadth: { advancing, declining, total: knownChanges.length },
    regime, session: currentSession(nowDate), recentEvents, riskAlerts, health, recentErrors,
    pipeline: input.pipeline ?? { stage: "using-okx", mode: "normal", tier: "l2", marketApiDurationMs: 0, binanceDurationMs: null, okxDurationMs: input.okx?.health.latencyMs ?? null, okxFetchedFields: [] },
  };
}
  ...
  for (const asset of stale.assets) {
    const mark = (candidate: unknown) => {
      if (candidate && typeof candidate === "object" && "state" in candidate) {
        ...
        if (value.state !== "missing") { value.state = "stale"; value.reason = "行情服務暫時不可用；顯示最後成功資料"; }
      }
    };
...
/** Merge L1/L2/L3 while retaining last good fields and v13-only timeframe data. */
export function mergeSnapshotsProgressive(prev: MarketHubPayload | null, next: MarketHubPayload): MarketHubPayload {
  if (!prev) return next;
  const prevMap = new Map(prev.assets.map((asset) => [asset.symbol, asset]));
  const nextMap = new Map(next.assets.map((asset) => [asset.symbol, asset]));
  ...
  const assets = symbols.map((symbol) => {
    ...
    if (!b) return a;
    const timeframes = Object.fromEntries(TIMEFRAMES.map((timeframe) => {
      const before = a.timeframes?.[timeframe];
      const after = b.timeframes?.[timeframe];
    ...
    return {
      ...
      change1h: pickMetric(a.change1h, b.change1h), change4h: pickMetric(a.change4h, b.change4h),
      ...
      strategies,
      setup: preferredSetup(strategies),
    };
  });
  const recentEvents = assets.flatMap((asset) => asset.events ?? []).sort((a, b) => b.occurredAt.localeCompare(a.occurredAt)).slice(0, 12);
  return {
    ...next, assets, recentEvents,
