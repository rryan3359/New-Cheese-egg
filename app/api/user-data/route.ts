...
const strategySchema = z.enum(["EMA Trend", "Bollinger Breakout", "ICT / SMC", "Trend Pullback", "Breakout", "Volatility Squeeze", "Funding Mean Reversion", "Positioning Divergence", "ICT Liquidity Sweep", "Range Mean Reversion"]);
const alertSchema = z.object({ id: z.string().min(1).max(100), symbol: z.string().regex(/^[A-Z0-9]{2,20}$/), type: z.enum(["price_target", "price_range", "breakout", "funding", "oi_change", "positioning_reversal", "strategy_eligible", "liquidity_sweep", "risk_reward", "provider_health"]), timeframe: timeframeSchema, strategy: strategySchema.nullable(), strategyVersion: z.number().int().positive().optional(), strategyLegacy: z.boolean().optional(), strategyModel: z.enum(["Reversal", "Continuation"]).nullable().optional(), strategyRuleset: z.enum(["v13-legacy", "v13.1"]).optional(), operator: z.enum(["above", "below", "inside"]), threshold: z.number().finite(), thresholdUpper: z.number().finite().nullable(), referenceValue: z.number().finite().nullable(), enabled: z.boolean(), cooldownMinutes: z.number().min(0).max(43_200), dedupeKey: z.string().max(500).nullable(), lastEvaluatedAt: z.string().nullable(), lastTriggeredAt: z.string().nullable(), triggerCount: z.number().int().min(0), currentStatus: z.enum(["watching", "triggered", "cooldown", "missing", "disabled"]), lastReason: z.string().max(1_000), createdAt: z.string() }).strict();
...
const journalSchema = z.object({ id: z.string().min(1).max(100), symbol: z.string().regex(/^[A-Z0-9]{2,20}$/), side: z.enum(["Long", "Short"]), strategy: strategySchema, strategyVersion: z.number().int().positive().optional(), strategyLegacy: z.boolean().optional(), strategyModel: z.enum(["Reversal", "Continuation"]).nullable().optional(), strategyRuleset: z.enum(["v13-legacy", "v13.1"]).optional(), timeframe: timeframeSchema, reason: z.string().max(5_000), entry: z.number().positive(), stop: z.number().positive(), target: z.number().nonnegative(), exit: z.number().positive(), quantity: z.number().positive(), fees: z.number().nonnegative(), fundingCost: z.number().finite(), actualPnl: z.number().finite(), rMultiple: z.number().finite(), followed: z.boolean(), mistake: z.string().max(200), notes: z.string().max(10_000), chartNote: z.string().max(2_000), tradeDate: z.string(), entryTime: z.string(), exitTime: z.string(), createdAt: z.string() }).strict();
const settingsSchema = z.object({ refreshSeconds: z.number().int().min(30).max(3_600), watchlist: z.array(z.string().regex(/^[A-Z0-9]{2,20}$/)).max(50), dailyLossLimit: z.number().nonnegative().max(100_000_000), defaultRiskPercent: z.number().positive().max(10), defaultFeeRate: z.number().nonnegative().max(.02), defaultSlippageRate: z.number().nonnegative().max(.02), minimumNetRr: z.number().min(1.5).max(10) }).strict();
