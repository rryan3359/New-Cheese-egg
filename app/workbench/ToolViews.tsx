...
import { STRATEGY_NAMES, TIMEFRAMES, type MarketHubPayload, type StrategyName, type Timeframe } from "../../lib/market/types";
...
const strategyNames: StrategyName[] = [...STRATEGY_NAMES];
...
export function AlertsView({ data, alerts, events, watchlist, persistence, onUpsert, onDelete }: { data: MarketHubPayload | null; alerts: AlertRule[]; events: AlertEvent[]; watchlist: string[]; persistence: "d1" | "device"; onUpsert: (rule: AlertRule) => void; onDelete: (id: string) => void }) {
  ...
  const [strategy, setStrategy] = useState<StrategyName>("EMA Trend");
...
export function JournalView({ journal, persistence, onUpsert, onDelete }: { journal: JournalEntry[]; persistence: "d1" | "device"; onUpsert: (entry: JournalEntry) => void; onDelete: (id: string) => void }) {
  ...
  const now = new Date();
  const [draft, setDraft] = useState({ symbol: "BTC", side: "Long" as "Long" | "Short", strategy: "EMA Trend" as StrategyName, timeframe: "15m" as Timeframe, reason: "", entry: 0, stop: 0, target: 0, exit: 0, quantity: 0.01, fees: 0, fundingCost: 0, followed: true, mistake: "無", notes: "", chartNote: "", tradeDate: now.toISOString().slice(0, 10), entryTime: now.toISOString(), exitTime: now.toISOString() });
