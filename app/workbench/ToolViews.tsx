...
export function HealthView({ data, fallbackTesting, onFallbackTest }: { data: MarketHubPayload; fallbackTesting: boolean; onFallbackTest: () => void }) {
  return <div className="view-stack"><section className="view-title with-action"><div><p>DATA HEALTH</p><h1>每個數字，都知道從哪裡來。</h1><span>即時、備援、稍早資料與暫缺會分開標示；拿不到的資料就顯示「—」，不會假裝是零。</span></div><button className="secondary-terminal-button" type="button" onClick={onFallbackTest} disabled={fallbackTesting}>{fallbackTesting ? "正在測試 OKX…" : "測試 OKX 備援"}</button></section>
    {fallbackTesting && <section className="fallback-progress" aria-live="polite"><i /><div><b>正在確認 OKX 是否可接手</b><span>目前畫面會保留，不會因測試而變成空白。</span></div></section>}
    <section className="pipeline-summary"><article><span>目前使用方式</span><b>{data.pipeline.mode === "force-okx" ? "手動測試 OKX" : "自動選擇最快來源"}</b></article><article><span>整體更新時間</span><b>{formatSeconds(data.pipeline.marketApiDurationMs)}</b></article><article><span>Binance 回應</span><b>{formatSeconds(data.pipeline.binanceDurationMs)}</b></article><article><span>OKX 回應</span><b>{data.pipeline.okxDurationMs === null ? "這次不需等待" : formatSeconds(data.pipeline.okxDurationMs)}</b></article></section>
