...
export function CockpitView({ data, watchlist, onNavigate, onOpenChart }: { data: MarketHubPayload; watchlist: string[]; onNavigate: (view: string) => void; onOpenChart: OpenChart }) {
  ...
  return <div className="view-stack">
    ...
    <section className="terminal-panel opportunity-panel"><div className="panel-heading"><div><p>OPPORTUNITY QUEUE</p><h2>正在形成的交易機會</h2></div></div><div className="opportunity-grid">{setups.length ? setups.map((setup) => <SetupCompact key={setup.id} setup={setup} onOpen={() => onOpenChart(setup.symbol, setup.timeframe, setup.strategy)} />) : <div className="inline-empty">目前沒有條件完成或接近完成的設定；等待也是有效決策。</div>}</div></section>
  </div>;
}
    ...
    <div className="view-stack">
      ...
      <section className="position-table">
        <div className="position-row position-head">
          ...
          <span>全體多空比</span><span>大戶多空比</span><span>傾向分數</span>
        </div>
...
export function ChartView({
  ...
  if (!asset) {
    return <EmptyState title="沒有圖表資料" copy="目前沒有可用的市場行情，請稍後重新整理。" />;
  }
  ...
  return (
    <div className="view-stack decision-view">
      <section className="view-title with-action">
        <div>
          <p>CHART WORKSPACE</p>
          <h1>{asset.symbol.replace("USDT", "/USDT")} 決策</h1>
        </div>
        <div className="chart-toolbar">
          ...
          >
            {symbolOptions.map((item) => (
              <option key={item.symbol} value={item.symbol}>
                {watchlist.includes(item.symbol) ? `★ ${item.symbol}` : item.symbol}
              </option>
            ))}
          </select>
          <select
          ...
          >
            {(Object.keys(strategyTips) as StrategyName[]).map((item) => (
      ...
      <section className="chart-layout decision-layout">
        ...
        <article className="terminal-panel strategy-plan decision-plan-card">
          <div className="panel-heading">
            <div>
              <p>TRADE PLAN</p>
              <h2>{setup ? strategyLabels[setup.strategy] : "策略資料不足"}</h2>
            </div>
            {setup && <StatePill state={setup.status} />}
          </div>
          {setup ? (
            <>
              <div className="plan-regime">
                <span className={`direction ${setup.direction.toLowerCase()}`}>
                  {directionLabels[setup.direction] ?? setup.direction}
                </span>
                <b>{setup.timeframe}</b>
                <em>
              ...
              <ul className="checklist">
                {setup.reasons.map((reason) => (
                  <li className="done" key={reason}>
                    ✓ {reason}
                  </li>
                ))}
                {setup.missingConditions.map((reason) => (
                  <li key={reason}>— {reason}</li>
                ))}
              </ul>
              <div className="formula-note">
                <b>簡單教學</b>
                <p>{strategyTips[setup.strategy]}</p>
              </div>
            </>
          ) : (
