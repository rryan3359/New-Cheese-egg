# Cheese&Egg v14 · 日內量化決策台

Cheese&Egg v14 延續原有 Sidebar、手機導覽、深淺主題、OKX Market Hub、D1、警報、風控、日誌、資料健康與漸進載入，並加入 30 個 OKX USDT 永續市場的觀察清單、市場數據與橫截面分析。首頁仍只呈現市場狀態、交易時段、事件與最多五個通過淨 RR 初篩的機會；沒有合格機會時會明確顯示「目前不適合交易／等待也是交易決策」。

## 免費市場情報

- 大戶籌碼壓力：比較 OKX 大戶帳戶多空比、大戶持倉多空比與全體帳戶多空比的歷史偏離，輸出 -100 到 +100 的相對分數。它不是錢包所有權或真實持倉集中度。
- 強平監測：讀取 OKX 公開的已成交強平事件，依官方合約面值換算可確認的 USDT 名目量；回傳失敗或無法換算時顯示 N/A。
- 波動雷達：同時比較 1h ATR／價格、BB Width、成交量 Z-score、1h 與 24h 漲跌。
- 市場廣度與橫截面：只對有效資料排名，缺值不參與百分位，也不補成 0。

強平頁顯示的是 OKX 最近回傳樣本，不是固定 24 小時統計，也不是未來清算地圖。所有功能只用公開資料，不需要付費供應商或交易所 API key。
